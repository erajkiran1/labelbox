...
def main():
    try:
        ...
        parser.add_argument("-f", "--folder_path", help="Please give Folder path in string format where you want to download",
                            required=True)
        parser.add_argument("-j", "--json_path", help="please give location path in string format of LABEL JSON FILE",
                            required=True)
        ...
        print(Fore.RED + "we are creating {} threads to download all files using threads".format(
            ceil(len(data) // 100)))
